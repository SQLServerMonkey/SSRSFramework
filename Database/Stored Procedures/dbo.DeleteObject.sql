...
--// This procedure has been customised in order to add a delete statement that will clean up the custom.ReportDetails table that has been added as part of the SSRS Framework.
-----------------------------------------
--// Changes Made by;
...
--// Blog:      http://www.jqmartin.co.uk
-----------------------------------------
--// Additional code has been flagged and can be found by searching for the word CUSTOMISATION.
...
--// ** Statement Added for Framework --Start--**
--// CUSTOMISATION
--// Delete custom report details records so as not to cause an FK viloation.
DELETE FROM [Custom].[ReportDetail]
    WHERE ItemID = (SELECT SSRSCatalogue.ItemID FROM dbo.[Catalog] AS SSRSCatalogue WHERE SSRSCatalogue.[Path] = @Path OR SSRSCatalogue.[Path] LIKE @Prefix ESCAPE '*')
;
--// ** Statement Added for Framework --End--**
